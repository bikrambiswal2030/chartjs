(function (global, factory) {
  ...
  const drawLinearAxis = lineArg => {
    const scale = linear$1()
  ...
  const drawBar = barArg => {
    ...
    const xScale = drawLinearAxis({
      type: barArg.type,
      domainMin: 0,
      domainMax: max(barArg.data, d => d.xAxis),
      rangeMin: 0,
  ...
  const drawLine = lineArg => {
    const xScale = drawBandAxis({
      type: lineArg.type,
      domainArr: lineArg.data.map(d => d.xAxis),
      rangeMin: 0,
    ...
    const yScale = drawLinearAxis({
      type: lineArg.type,
      domainMin: 0,
      domainMax: max(lineArg.data, d => d.yAxis),
  ...
  const drawPie = pieArg => {
    const pie$1 = pie()(pieArg.pieData.map(d => d.yAxis));
    ...
    const color = ordinal(Set1);

    const arc$1 = pieArg.graph
      .selectAll(".arc")
      .data(pie$1)
      .enter()
      .append("g")
  ...
  const drawColumn = columnArg => {
    ...
    const yScale = drawLinearAxis({
      type: columnArg.type,
      domainMin: 0,
      domainMax: max(columnArg.data, d => d.yAxis),
    ...
    switch (data.type) {
      case "line":
        drawLine({
          type: data.type,
          data: data.value,
          xScaleAttrName: "xAxis",
          yScaleAttrName: "yAxis",
