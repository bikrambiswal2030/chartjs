...
var processData = function(jsonData) {
  ...
  var date;
  var locale = "en-us";
  var months = Object.keys(jsonVal)
    .map(function(item) {
      date = new Date(item);
  ...
  Object.keys(jsonVal).filter(function(item) {
    ...
    orderClosingByMonth[thisMonth]["allValue"].push(
      parseFloat(jsonVal[item]["4. close"])
    );
...
$element.onclick = function(event) {
  ...
  if (activePoints.length > 0) {
    ...
    if (labels.indexOf(label) != -1) {
      ...
      myChart.data.datasets[0].data = orderClosingByMonth[
        label
      ].allValue.reverse();
      myChart.update();
